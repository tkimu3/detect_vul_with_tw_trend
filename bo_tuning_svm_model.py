...
# results directory
res_dir = 'rbf_bo_tuning_SVM_model_results'
...
for i, filepath in enumerate(filePaths):
    ...
    with open (f'{res_path}/{file_stem}_bo_result.json','w') as f:
        json.dump(svm_bo.res, f, indent=4)
    print("cv_results_ record ended")

    best_params = {k: np.power(10, v) for k, v in svm_bo.max['params'].items()}
    best_score = svm_bo.max['target']
    ...
    with open (f'{res_path}/{file_stem}_rbf_bo_result.txt',mode='a') as f:
        print(msg1, file=f)
        print(msg2, file=f)
